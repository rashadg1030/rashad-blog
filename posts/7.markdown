...
I'm using the `ReaderT` monad transformer\[2\] in combination with the `IO` monad. This is just the `Reader` monad with the addition of being able to return values in an `IO` context. The `ReaderT` is wrapped in a newtype so we can derive useful methods from other typeclasses. In short, the type definition above means `App` is a monad that can read from an environment of type `AppEnv` and return values of `IO a`. Note the various typeclasses that `App` derives. `App` has access to methods ranging from the useful basics like `fmap` (from `Functor`), to more powerful methods like `liftIO` (from `MonadIO`) and `ask` (from `MonadReader AppEnv`).

Wait, how are errors taken care of without `throwError` and `catchError` from `MonadError`? The `App` type has those methods too, but they to be defined manually becuase of the way the `AppError` type is defined:
...
This way of structuring an application around the `ReaderT` monad is known as the "ReaderT design pattern"\[3\], made popular by Michael Snoyman at FPComplete. If you think about it, an application runs in a context where:

1. It has access to an envrionment (in our case, the `AppEnv` type) where it can retrieve neccessary resources like a database connection, a configuration file, or TLS manager

and

2. Can perform input/output (i.e. writing to a database, downloading a file off the Internet, etc.)

The `ReaderT` monad transfomer in combination with the `IO` monad allows us to express this in our program.
...
1. We have the type synonym `AppEnv` that you saw in the definition of the `App` monad above. It's just the `Env` type parameterized by the `App` type. This was kind      of odd to me at first beacuse the definition is recursive; `AppEnv` is used to define `App` and `App` is used to define `AppEnv`.
...
3. This is the `Env` type. It's a record type that represents the different resources our `App` monad has access to. Interestingly, it has a type variable of kind         `Type -> Type`\[3\] meaning `m` must be filled in by a type that takes another type. This is type variable is primarily used for the `LogAction` type from the          [`co-log`]() library.

4. An instance of the `HasLog` typeclass from the `co-log` library for our `Env` type.

5. The `Has` typeclass. It is parameterized by two type variables; the `field` of interest and the `env` it is retrieved from. It only has one method, `obtain`, which     takes a value of type `env` and gives back a value of type `field`. This is an example of the Has typeclass pattern\[4\]. This pattern is commonly used in              conjuction with the ReaderT design pattern to make it even more clean and extensible.

6. Here are our instances of the `Has` typeclass. We add one for each field in the `Env` type. The two definintions basically mean we have a way of obtaining a            `DbPool` and `LogAction` from our `Env` type.
...
This solution isn't one hundered percent correct because there will be cases where the `.cabal` file isn't stored on GitHub at all, the `.cabal` file isn't in the `master` branch, or we're dealing with a multi-package Haskell project. For now, we're ignoring these cases, but we definitely plan on adding support for these kinds of Haskell projects in the future to ensure that we have as much useful metadata as possible.
...
class Monad m => MonadDownload m where
    downloadFile :: Url -> m BysteString
~~~
...
1. You may be wondering why `downloadFile` is being implemented via the `downloadFileImpl` function below it.
   This is actually useful because we can export the `downloadFileImpl` function for testing without worrying about `MonadDownload`.
...
3. Finally, the actual implementation of `downloadFile`. After asking around on [r/haskell]() for a library that would allow us to idiomatically download files from       URLs, we decided to use the [http-client-tls](https://hackage.haskell.org/package/http-client-tls) library. From this library we're using `newTlsManager` for           loading up a new TLS manager and `httpLbs` for making the request. We use the response status code for logging messages and return the response body.
...
A new TLS manager has to be created each time we call `downloadFileImpl`. This is unneccessary and can lead to suboptimal performance, so we decided to add a field for the manager to our `Env` record. This way, we only need to create the manager once the application is started and grab the same one each time we need to download a file.
...
Now, the manager is only created once and is grabbed from the envrionment anytime it's needed to download a file. Nice!

## Conclusion

Issue-wanted is looking better and better as the Summer goes on, and I'm excited to see what it will become. Up until the past couple of weeks, most of the code in issue-wanted was borrowed from the Holmusk [three-layer]() template project and I haven't really got the opportunity to implement anything unique to issue-wanted. This is rapidly starting to change as I move past building the foundation and work my way into the more interesting features, like the one I wrote about in this post, that will make issue-wanted what it is.
